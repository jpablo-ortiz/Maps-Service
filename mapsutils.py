...
class MapService(object):
    ...
    def __init__(self, api_bingmaps_key, version=1):
        """Instanciar un objeto MapService con la clave de la API de Bing Maps.
    ...
    def _tuple_LatLng_to_string(self, latlng):
    ...
    def _posicion_a_string_url(self, ubicacion):
        ...
        if type(ubicacion) is tuple or type(ubicacion) is list:
            return self._tuple_LatLng_to_string(ubicacion)
    ...
    def _rest_ruta(self, inicio, final, via=[], **kwargs):
        ...
        paradas = []
        paradas.append(self._posicion_a_string_url(inicio))
        if len(via) > 0:
            paradas = paradas + \
                [self._posicion_a_string_url(latlng) for latlng in via]
        paradas.append(self._posicion_a_string_url(final))
    ...
    def _rest_localizacion_imagen(self, ubicacion, width=500, height=500, zoomLevel=16,  **kwargs):
        ...
        if type(ubicacion) is tuple or type(ubicacion) is list:
            # Se agrega el valor obligatorio a enviar
            url += '/' + self._tuple_LatLng_to_string(ubicacion) + '?'

            # Para poder agregar un pin en la ubicación
            kwargs.update(
                {'pushpin': self._tuple_LatLng_to_string(ubicacion) + ';66'})
        ...
        filename = 'images/localizacion(' + \
            self._posicion_a_string_url(ubicacion) + ').png'
    ...
    def _rest_ruta_imagen(self, inicio, final, via=[], width=500, height=500, **kwargs):
        ...
        path = Path("images")
        path.mkdir(parents=True, exist_ok=True)
        filename = 'images/Ruta(' + self._posicion_a_string_url(inicio) + \
            ')(' + self._posicion_a_string_url(final) + ').png'
